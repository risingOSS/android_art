...
static constexpr size_t kMutatorCompactionBufferCount = kIsDebugBuild ? 128 : 256;
    ...
    if (bump_pointer_space_->HasAddress(reinterpret_cast<mirror::Object*>(fault_page))) {
      ...
      if (minor_fault_initialized_) {
        ...
        if (buf == nullptr) {
          ...
          CHECK_LT(idx, kMutatorCompactionBufferCount);
          // The first buffer is used by GC-thread.
          buf = compaction_buffers_map_.Begin() + (idx + 1) * kPageSize;
...
void MarkCompact::CompactionPhase() {
  ...
  // userfaultfd from moving-space and then zapping from-space. If a mutator (or
  // uffd worker thread) starts processing a moving-space page after this, it will
  // see that page already completed and hence do nothing.
  for (uint32_t i = 0; compaction_in_progress_count_.load(std::memory_order_relaxed) > 0; i++) {
    BackOff(i);
  }
